...
export function License({ accessToken, userProfile, licenseInfo, onLicenseUpdated }: LicenseProps) {
  ...
  useEffect(() => {
    console.log('License component mounted/updated with licenseInfo:', licenseInfo)
  ...
  const handlePurchase = async (planId?: string) => {
    ...
    try {
      ...
      if (selectedCountry === 'colombia') {
        ...
        toast.loading('Redirigiendo a PSE...', { id: 'payment-process' })
        
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-4d437e50/license/pse/create`,
          {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${accessToken}`,
        ...
        if (data.success && data.paymentUrl) {
          ...
          // For demo purposes, directly upgrade the plan
          ...
          if (upgradeData.success) {
            ...
            toast.error('Error al actualizar el plan', {
              description: upgradeData.error || 'Error desconocido'
            })
          }
        } else {
          toast.dismiss('payment-process')
        ...
        toast.info('Paddle Integration', {
          description: 'En producción se integrará con Paddle para pagos internacionales'
        })
        
        // For now, directly upgrade (in production, this would redirect to Paddle)
        const upgradeResponse = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-4d437e50/license/upgrade-plan`,
          {
            method: 'POST',
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-background p-4 sm:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl mb-3 text-gray-900 dark:text-gray-100">Gestión de Licencia</h1>
          <p className="text-base text-gray-600 dark:text-gray-400">
            Administra tu suscripción y actualiza tu plan según las necesidades de tu negocio
          </p>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                <div className="flex items-start gap-3">
                  <Calendar className="text-blue-600 dark:text-blue-400 flex-shrink-0 mt-1" size={24} />
                    ...
                    {companyData?.licenseExpiry ? (
                          ...
                          {daysRemaining > 7 ? (
                            <>
                              ...
                              <p className="text-sm text-blue-700 dark:text-blue-300">
                                Quedan <strong>{daysRemaining} días</strong> hasta la renovación
                              </p>
                            </>
                          ) : daysRemaining > 0 ? (
                            <>
                              ...
                              <p className="text-sm text-orange-700 dark:text-orange-300">
                                ⚠️ Quedan solo <strong>{daysRemaining} días</strong> - Renueva pronto
                              </p>
                            </>
                          ) : (
                            <>
                      ...
                      <>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <p className="text-sm text-blue-700 dark:text-blue-300">
                            Haz clic en "Recargar Datos de Licencia" para inicializar tu licencia
                          </p>
                        </div>
                      </>
                    )}
                    {inTrial && (
                      <div className="mt-2 bg-orange-100 dark:bg-orange-950 border border-orange-300 dark:border-orange-800 rounded px-3 py-2">
                        <p className="text-xs text-orange-800 dark:text-orange-200">
                          <strong>Período de Prueba:</strong> Disfruta de todas las funcionalidades. 
                          Al finalizar, deberás renovar para continuar usando el sistema.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-semibold text-gray-900 dark:text-gray-100">{currentPlan.limits.branches}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-semibold text-gray-900 dark:text-gray-100">{currentPlan.limits.admins}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-semibold text-gray-900 dark:text-gray-100">{currentPlan.limits.advisors}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-semibold text-gray-900 dark:text-gray-100">{currentPlan.limits.technicians}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border dark:border-gray-700 rounded-lg p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-gray-100">PSE - Colombia</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Pagos Seguros en Línea</p>
                  </div>
                </div>
              ...
              <div className="border dark:border-gray-700 rounded-lg p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-gray-100">Paddle - Internacional</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Pagos Globales</p>
                  </div>
                </div>
                <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  <li className="flex items-start gap-2">
        ...
        <Tabs defaultValue="plans" className="mb-8">
          ...
          <TabsContent value="plans" className="space-y-8">
            ...
            <Card>
              ...
              <CardContent>
                <RadioGroup value={selectedCountry} onValueChange={(val) => setSelectedCountry(val as any)}>
                ...
                >
                  ...
                  <Label htmlFor="colombia" className="flex-1 cursor-pointer">
                    <div className="flex items-center gap-2">
                      <MapPin size={20} className="text-blue-600 dark:text-blue-400" />
                      <div>
                        <p className="font-medium text-gray-900 dark:text-gray-100">Colombia (PSE)</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Precios en COP</p>
                      </div>
                    </div>
                  </Label>
                </div>
                ...
                >
                  ...
                  <Label htmlFor="international" className="flex-1 cursor-pointer">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium text-gray-900 dark:text-gray-100">Internacional (Paddle)</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Precios en USD</p>
                      </div>
                    </div>
                  </Label>
                </div>
          ...
          <CardContent>
            ...
            {showValidation && validationResult && !validationResult.canChange && (
              <Alert variant="destructive" className="mb-6">
                ...
                <AlertDescription>
                  <p className="font-semibold mb-2">No puedes cambiar a este plan porque excedes los límites:</p>
                  ...
                  <p className="mt-3 text-sm">
                    <strong>Ve a Configuración → Gestión de Usuarios y Sucursales</strong> para eliminar los recursos excedentes.
                  </p>
                </AlertDescription>
              </Alert>
            )}
                ...
                >
                    ...
                    <div className="mb-4">
                      {selectedCountry === 'colombia' ? (
                        <>
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">COP / mes</p>
                        </>
                      ) : (
                        <>
                          <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">USD / mes</p>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4 pb-4 border-b dark:border-gray-700">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">{plan.limits.branches}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400 flex items-center gap-2">
                        <UserCog size={16} />
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">{plan.limits.admins}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400 flex items-center gap-2">
                        <Users size={16} />
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">{plan.limits.advisors}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400 flex items-center gap-2">
                        <Wrench size={16} />
                      ...
                      <span className="font-semibold text-gray-900 dark:text-gray-100">{plan.limits.technicians}</span>
                    </div>
                  </div>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-800 dark:to-blue-950 rounded-lg p-6 border dark:border-gray-700">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900 dark:text-gray-100">{selectedPlanData?.name}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Método: {selectedCountry === 'colombia' ? 'PSE (Colombia)' : 'Paddle (Internacional)'}
                  </p>
                </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    {currentPlan && plans.findIndex(p => p.id === selectedPlan) > plans.findIndex(p => p.id === currentPlan.id) 
                      ? 'Mejorar Plan Ahora' 
                      : 'Cambiar Plan Ahora'}
                  </>
                )}
              ...
              {selectedPlan !== currentPlan?.id && (
                <div className="mt-3 bg-blue-100 dark:bg-blue-900 rounded-lg p-3">
                  <p className="text-xs text-blue-900 dark:text-blue-100 flex items-start gap-2">
                    ...
                    <span>
                      Al cambiar de plan, los nuevos límites se aplicarán inmediatamente y tu fecha de renovación se actualizará.
                    </span>
                  </p>
                </div>
              )}
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Si tienes preguntas sobre los planes, necesitas un plan personalizado para tu negocio, 
              o tienes problemas con tu pago, no dudes en contactarnos.
            </p>
